...
A comprehensive web technology detection system that combines multiple detection engines,
user agents, and datasets for maximum accuracy and reliability.
"""
...
from .enhanced_tech_detector import EnhancedTechDetector, AnalysisResult
from ..integrations.dynamic_whatweb_integration import DynamicWhatWebIntegration, merge_whatweb_results
...
class UltimateTechDetector:
    """Technology detection system with all features"""
    
    def __init__(self, datasets_dir: str = "json_datasets"):
        self.datasets_dir = datasets_dir
        self.detector = EnhancedTechDetector(datasets_dir)
        self.whatweb = None
    ...
    def _initialize_engines(self):
        ...
        # Initialize Dynamic WhatWeb
        try:
            self.whatweb = DynamicWhatWebIntegration()
            logger.info("Dynamic WhatWeb integration initialized")
        except Exception as e:
            logger.warning(f"WhatWeb integration failed: {e}")
        
        # Initialize Wappalyzer
        try:
            # Use built-in Wappalyzer.latest() for maximum technology coverage
            self.wappalyzer = WappalyzerIntegration()
            logger.info("Wappalyzer integration initialized with latest data")
        except Exception as e:
    ...
    async def analyze_url(self, url: str, options: Dict[str, Any] = None) -> AnalysisResult:
        """Analyze URL with all available engines and user agents"""
        options = options or {}
        start_time = time.time()
        
        logger.info(f"Starting ultimate analysis of {url}")
        
        # Get base analysis from enhanced detector
        ...
        logger.info(f"Ultimate analysis completed in {total_time:.2f}s - {len(base_result.technologies)} technologies detected")
        
        return base_result
    
    def _post_process_analysis(self, result: AnalysisResult, options: Dict[str, Any]) -> AnalysisResult:
        ...
        result.metadata['summary'] = {
            ...
            'by_user_agent': self._count_by_user_agent(result.technologies),
    ...
    def _count_by_user_agent(self, technologies: List[Any]) -> Dict[str, int]:
        """Count technologies by user agent"""
        counts = {}
        for tech in technologies:
            ua = tech.user_agent_used
            if ua:
                ua_short = ua[:50] + "..." if len(ua) > 50 else ua
                counts[ua_short] = counts.get(ua_short, 0) + 1
        return counts
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Ultimate Web Technology Detection System",
...
Examples:
  ...
  # Debug mode
  python tech_detector.py https://example.com --debug --verbose --dump
        """
    )
    ...
    engine_group.add_argument('--use-dataset', action='store_true', default=True, 
                            help='Use enhanced dataset detection (default: enabled)')
    ...
    # User agent options
    ua_group = parser.add_argument_group('User Agent Options')
    ua_group.add_argument('--user-agent', help='Custom user agent string')
    ua_group.add_argument('--user-agents', type=int, default=3, 
                         help='Number of user agents to try (default: 3)')
    ua_group.add_argument('--preferred-browser', choices=['chrome', 'firefox', 'safari', 'edge', 'random'], 
                         default='chrome', help='Preferred browser (default: chrome)')
    ua_group.add_argument('--preferred-os', choices=['windows', 'macos', 'linux', 'mobile'], 
                         default='windows', help='Preferred OS (default: windows)')
...
def format_output(result: AnalysisResult, output_format: str, pretty: bool = False) -> str:
    """Format output in specified format"""
    if output_format == 'json':
        return json.dumps(result.__dict__, indent=2 if pretty else None, ensure_ascii=False, default=str)
        ...
        output += f"User Agents Tried: {len(result.user_agents_tried)}\n"
        output += f"Successful Agent: {result.successful_agent[:50] if result.successful_agent else 'None'}...\n\n"
        ...
        if result.technologies:
            for tech in result.technologies:
                ...
                output += f"  User Agent: {tech.user_agent_used[:50] if tech.user_agent_used else 'N/A'}...\n\n"
        else:
...
async def main():
    ...
    # Initialize detector
    detector = UltimateTechDetector(args.datasets_dir)
    ...
    options = {
        ...
        'preferred_os': args.preferred_os,
        'dump': args.dump
    }
    ...
    if args.user_agent:
        ...
        options['user_agents'] = args.user_agents
    ...
    try:
        if args.batch:
            ...
            # Output results
            if args.output == 'json':
                output = json.dumps([result.__dict__ for result in results], indent=2 if args.pretty else None, ensure_ascii=False, default=str)
            else:
